...
class CNN3D(nn.Module):
    def __init__(self, in_channels=30, num_classes=16, use_attention=True):
        ...
        # 原始输入 (B, 1, C, H, W) → 先 reshape 为 (B, C, 1, H, W)，模拟跳过光谱卷积
        self.input_bn = nn.BatchNorm3d(in_channels)  # 将 C 维作为通道维，直接输入

        self.spatial_conv = nn.Conv3d(in_channels, 8, kernel_size=(1, 3, 3), padding=(0, 1, 1))
    ...
    def forward(self, x):  # x: (B, 1, C, H, W)
        ...
        x = self.spatial_conv(x)  # → (B, 8, 1, H, W)
        x = self.bn1(x)
        x = self.relu(x)
...
def evaluate_and_log_metrics(y_true, y_pred, model, run_seed, dataset_name, acc, train_time=None, val_time=None):
    log_dir = f"ablation_logs/wo_spectral/{dataset_name}/seed_{run_seed}"
...
def train_and_evaluate(run_seed=42, dataset_name="Botswana"):
    set_seed(run_seed)
    data_path = '../../root/data/HSI_dataset/Botswana/Botswana.mat'
    label_path = '../../root/data/HSI_dataset/Botswana/Botswana_gt.mat'
    data = scio.loadmat(data_path)['Botswana']
    label = scio.loadmat(label_path)['Botswana_gt'].flatten()
    h, w, bands = data.shape
    data_reshaped = data.reshape(h * w, bands)

    # 先拉平 label_map，筛选有效像素
    valid_mask = label > 0
    valid_pixels = data.reshape(-1, bands)[valid_mask]
    valid_labels = label[valid_mask] - 1  # 类别从0开始

    # train/val/test 划分
    X_train_full, X_test, y_train_full, y_test = train_test_split(
        valid_pixels, valid_labels, test_size=0.15, stratify=valid_labels, random_state=42
    )
    ...
    train_loader = DataLoader(TensorDataset(torch.from_numpy(X_train_pca), torch.from_numpy(y_train)),
                              batch_size=128, shuffle=True)
    val_loader = DataLoader(TensorDataset(torch.from_numpy(X_val_pca), torch.from_numpy(y_val)),
                            batch_size=128, shuffle=False)

    test_loader = DataLoader(TensorDataset(torch.from_numpy(X_test_pca), torch.from_numpy(y_test)),
                             batch_size=128, shuffle=False)
    ...
    # 保存图像
    fig_path = os.path.join(log_dir, f"{dataset_name}_run{run_seed}_vis.png")
