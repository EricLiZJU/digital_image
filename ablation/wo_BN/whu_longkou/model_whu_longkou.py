...
class CNN3D(nn.Module):
    def __init__(self, in_channels=30, num_classes=16, use_attention=True, use_bn=False):
...
def evaluate_and_log_metrics(y_true, y_pred, model, run_seed, dataset_name, acc, train_time=None, val_time=None):
    log_dir = f"ablation_logs/wo_BN/{dataset_name}/seed_{run_seed}"
...
def train_and_evaluate(run_seed=42, dataset_name="whu_longkou"):
    set_seed(run_seed)
    data_path = '../../root/data/HSI_dataset/Matlab_data_format/Matlab_data_format/WHU-Hi-LongKou/WHU_Hi_LongKou.mat'
    label_path = '../../root/data/HSI_dataset/Matlab_data_format/Matlab_data_format/WHU-Hi-LongKou/WHU_Hi_LongKou_gt.mat'
    hyperspec = loadmat(data_path)
    data = hyperspec['WHU_Hi_LongKou']
    data = np.transpose(data, (1, 2, 0))
    gt = loadmat(label_path)
    label_map = gt['WHU_Hi_LongKou_gt']
