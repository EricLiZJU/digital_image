...
class CNN3D(nn.Module):
    def __init__(self, in_channels=30, num_classes=16, use_attention=True):
        ...
        self.spectral_conv = nn.Conv3d(1, 8, kernel_size=(7,1,1), padding=(3,0,0))  # 仅光谱卷积
    ...
    def forward(self, x):  # x: (B, 1, C, H, W)
        x = self.spectral_conv(x)         # (B, 8, C, H, W) → 原始空间结构未变
        x = self.bn1(x)
        x = self.relu(x)
...
def evaluate_and_log_metrics(y_true, y_pred, model, run_seed, dataset_name, acc, train_time=None, val_time=None):
    log_dir = f"ablation_logs/wo_spatial/{dataset_name}/seed_{run_seed}"
