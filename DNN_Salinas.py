...
for i in range(label.shape[0]):
    if label[i] != 0:
        count[label[i] - 1].append(data_reshaped[i])
...
for loop in range(loops):
    ...
    for epoch in range(epochs):
        ...
        for index, (images, labels) in enumerate(data_loader_train):
            ...
            loss_train.append(loss.data_reshaped.item())
            if loops == 1:
        ...
        if (epoch + 1) % 10 == 0:  # 每训练10个epoch保存一次模型
            ...
            torch.save(optimizer.state_dict(), './models/DNN_NOBG_optimizer_' + str(epoch + 1) + '_salinas.pth')
        ...
        for index, (images, labels) in enumerate(data_loader_test):
            ...
            labels_cuda = Variable(labels).to(device)  # 将标签载入cuda
            loss = lossFun(CC_output, labels_cuda)  # 计算误差
            loss_test.append(loss.data_reshaped.item())
    ...
    torch.save(CC.state_dict(), './models/DNN_NOBG_final_salinas.pth')
    torch.save(optimizer.state_dict(), './models/DNN_NOBG_optimizer_final_salinas.pth')
    ...
    plt.savefig("figures/Salinas_DNN_loss.pdf", bbox_inches='tight')
    plt.show()
    ...
    weights_net = torch.load(
        './models/DNN_NOBG_' + str(
            epoch_best) + '_salinas.pth', map_location='cpu')
