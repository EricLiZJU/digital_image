...
def train_and_evaluate_sgmae(run_seed=42,
    ...
    if 'Botswana' in data:
        img = data['Botswana']
        gt = label['Botswana_gt'].flatten()
    else:
        img = next(v for v in data.values() if isinstance(v, np.ndarray))
        gt = next(v for v in label.values() if isinstance(v, np.ndarray)).flatten()

    h, w, bands = img.shape
    label_map = gt.reshape(h, w)
    ...
    for i in range(h):
        for j in range(w):
            ...
            with torch.no_grad():
                if use_graph and gcn_logits_all is not None:
                    # 推理阶段若启用图头，默认仍用 CNN 单样本推理（因为图头需要全图）
    ...
    fig_path = os.path.join(log_root, f"{dataset_name}_SGMAE_run{run_seed}_vis.png")
    fig_path_pdf = os.path.join(log_root, f"{dataset_name}_SGMAE_run{run_seed}_vis.pdf")
