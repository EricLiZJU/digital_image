...
class CNN3D(nn.Module):
    def __init__(self, in_channels=30, num_classes=16):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv3d(1, 8, kernel_size=(3, 3, 3), padding=(1, 1, 1)),  # 小卷积核
            nn.ReLU(),
            nn.BatchNorm3d(8),
            nn.Conv3d(8, 16, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.BatchNorm3d(16),
            nn.AdaptiveAvgPool3d((1, 1, 1))  # 输出为 (16, 1, 1, 1)
        )
...
def train_and_evaluate(run_seed=42):
    ...
    plt.savefig(f"figures/3DCNN_run{run_seed}_ease_convkernal.pdf", bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    os.makedirs("figures", exist_ok=True)
    plt.savefig(f"figures/3DCNN_loss_seed{run_seed}_ease_convkernal.pdf", bbox_inches='tight')
    plt.close()
...
with open("results/3dcnn_repeat_results_ease_convkernal.txt", "w") as f:
    for i, acc in enumerate(accuracies):
