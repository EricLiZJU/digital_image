...
class CNN3D(nn.Module):
    def __init__(self, in_channels=30, num_classes=16):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv3d(1, 8, kernel_size=(7, 3, 3), padding=(0, 1, 1)),  # → (8, C-6, H, W)
            nn.ReLU(),
            nn.BatchNorm3d(8),
            nn.Conv3d(8, 16, kernel_size=3, padding=1),               # → (16, C-6, H, W)
            nn.ReLU(),
            nn.BatchNorm3d(16)
            # ⚠️ 此处去掉 AdaptiveAvgPool3d
        )
        # 原本 AdaptiveAvgPool3d 输出是 (16, 1, 1, 1)，现在保持全尺寸
        # 假设 patch_size=7，spectral_channels=30 → C_out = 24 (30-6)
        self.fc_layers = nn.Sequential(
            nn.Flatten(),        # Flatten 后维度为 16 * 24 * 7 * 7 = 18816
            nn.Linear(16 * 24 * 7 * 7, 256),
            nn.ReLU(),
            nn.Dropout(0.5),
...
def train_and_evaluate(run_seed=42):
    ...
    plt.savefig(f"figures/3DCNN_run{run_seed}_without_adaptAvgPool.pdf", bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    os.makedirs("figures", exist_ok=True)
    plt.savefig(f"figures/3DCNN_loss_seed{run_seed}_without_adaptAvgPool.pdf", bbox_inches='tight')
    plt.close()
...
with open("results/3dcnn_repeat_results_without_adaptAvgPool.txt", "w") as f:
    for i, acc in enumerate(accuracies):
