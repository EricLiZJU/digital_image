...
class CNN3D(nn.Module):
    def __init__(self, in_channels=30, num_classes=16):
        ...
        self.conv_layers = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm3d(16),
            nn.Conv3d(16, 32, kernel_size=3, padding=1),               # 增加层
            nn.ReLU(),
            nn.BatchNorm3d(32),
...
def train_and_evaluate(run_seed=42):
    ...
    plt.savefig(f"figures/3DCNN_run{run_seed}_deep.pdf", bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    os.makedirs("figures", exist_ok=True)
    plt.savefig(f"figures/3DCNN_loss_seed{run_seed}_deep.pdf", bbox_inches='tight')
    plt.close()
...
with open("results/3dcnn_repeat_results_deep.txt", "w") as f:
    for i, acc in enumerate(accuracies):
