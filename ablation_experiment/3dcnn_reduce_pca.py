...
def train_and_evaluate(run_seed=42):
    ...
    data_cube = data_pca.reshape(h, w, 15)  # shape: (145, 145, 30)
    ...
    patches, patch_labels = extract_3d_patches(
        data_cube, label_map,
        patch_size=7, spectral_channels=15  # 注意要和前面一致
    )
    ...
    print(device)
    model = CNN3D(in_channels=15, num_classes=16).to(device)
    ...
    plt.savefig(f"figures/3DCNN_run{run_seed}_reduce_pca.pdf", bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    os.makedirs("figures", exist_ok=True)
    plt.savefig(f"figures/3DCNN_loss_seed{run_seed}_reduce_pca.pdf", bbox_inches='tight')
    plt.close()
...
with open("results/3dcnn_repeat_results_reduce_pca.txt", "w") as f:
    for i, acc in enumerate(accuracies):
