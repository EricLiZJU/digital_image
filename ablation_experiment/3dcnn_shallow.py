...
def train_and_evaluate(run_seed=42):
    ...
    plt.savefig(f"figures/3DCNN_run{run_seed}_shallow.pdf", bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    os.makedirs("figures", exist_ok=True)
    plt.savefig(f"figures/3DCNN_loss_seed{run_seed}_shallow.pdf", bbox_inches='tight')
    plt.close()
...
with open("results/3dcnn_repeat_results_shallow.txt", "w") as f:
    for i, acc in enumerate(accuracies):
